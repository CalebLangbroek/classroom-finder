...
//@Query ("""
//                    SELECT times.day, times.start_time
//                    FROM saved_sections INNER JOIN sections ON crn = saved_sections.section_crn
//                    INNER JOIN courses ON course_id = courses.id
//                    INNER JOIN times on crn = times.section_crn ORDER BY start_time ASC
//                    """)
//fun getAlarms(): Cursor

    /**SELECT times.day, times.start_time
     * NOTE: ADD '%' before and after date
     *
     * """
    SELECT sections.crn AS section_crn, sections.course_id AS section_course_id, sections.title AS section_title,
    courses.id AS course_id, courses.title AS course_title, courses.subject AS course_subject,
...
//    fun getAlarms(date: String, time : String): Cursor
